...
private[spark] class BlockManager(
  ...
  /**
   * Put thees given block according to the given level in one of the block stor, replicating
  ...
  private def doPut(
    ...
    // todo 启动一个线程在本地存储之前，异步初始化好要进行的备份的数据，这有助于提高发送数据的速度
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        //todo 根据storeLevel判断使用哪种Blockstore，以及是否返回put操作的值
        ...
        //todo 如果使用的内存，则要将溢出的部分添加到updatedBlocks中
        if (putLevel.useMemory) {
